...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        verify = action(get_str('verifyImg'), self.RGB2HSV,
        ...
        sav1= action(get_str('sav1'), self.sav1,
                              '1', 'sav1', get_str('sav1'))
        sav2= action(get_str('sav2'), self.sav2,
                             '2', 'sav2', get_str('sav2'))
        sav3= action(get_str('sav3'), self.sav3,
                             '3', 'sav3', get_str('sav3'))

        sav4= action(get_str('sav4'), self.sav4,
                             '4', 'sav4', get_str('sav4'))
    
        sav5= action(get_str('sav5'), self.sav5,
                             '5', 'sav5', get_str('sav5'))
        sav6= action(get_str('sav6'), self.sav6,
                             '6', 'sav6', get_str('sav6'))
        sav7= action(get_str('sav7'), self.sav7,
                             '7', 'sav7', get_str('sav7'))
        sav8= action(get_str('sav8'), self.sav8,
                             '8', 'sav8', get_str('sav8'))
        sav9= action(get_str('sav9'), self.sav9,
                             '9', 'sav3', get_str('sav9'))
        ...
        add_actions(self.menus.file,
                    (open, open_dir, change_save_dir, open_annotation, copy_prev_bounding, self.menus.recentFiles, save, save_format, save_as, close, reset_all, delete_image, quit,sav1,sav2,sav3,sav4,sav5,sav6,sav7,sav8,sav9))
    ...
    def load_file(self, file_path=None):
        ...
        if file_path is None:
            ...
            print("SETTING_FILENAME", file_path)
    ...
    def sav_template(self,quick_button):
        if self.default_save_dir is not None and len(ustr(self.default_save_dir)):
           if self.file_path:
               image_file_name = os.path.basename(self.file_path)
               saved_file_name = os.path.splitext(image_file_name)[0]
               saved_path = os.path.join(
                   ustr(self.default_save_dir), saved_file_name)
               new_save_dir=os.path.join(ustr(self.default_save_dir),quick_button)

               #創建新的子資料夾存放新的jpg與txt
               if not os.path.isdir(new_save_dir):
                   os.mkdir(new_save_dir)
               save_path_jpg=saved_path+".jpg"
               save_path_txt=saved_path+".txt"
               
               # 複製jpg過去
               if( os.path.isfile(save_path_jpg ) ):
                   shutil.copyfile(save_path_jpg, os.path.join(new_save_dir, saved_file_name+".jpg"))
                   print("-- Copy:",save_path_jpg)
               else:
                   print( os.path.join(new_save_dir, saved_file_name) , " is not exist!")
                   
               #如果存在txt,則複製
               if( os.path.isfile(save_path_txt ) ):
                   shutil.copyfile(save_path_txt, os.path.join(new_save_dir, saved_file_name+".txt"))
                   print("-- Copy:",save_path_txt)
               else:
                   print( os.path.join(new_save_dir, saved_file_name+".txt") , " is not exist!")
